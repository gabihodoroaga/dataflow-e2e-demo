...
class PubsubResourceExtension : BeforeAllCallback, AfterAllCallback {
  ...
  companion object {
    ...
    fun createPubsubSubscription(projectId: String, topicId: String): String? {
      ...
      try {
        ...
        logger.info("Error creating subscription for project {}, topic {}.", projectId, topicId, e)
        return null
      } finally {
  ...
  override fun beforeAll(context: ExtensionContext) {
    ...
    val pubsubTopicField = testInstance::class.memberProperties.find { it.name == "pubsubTopic" }
    if (pubsubTopicField != null && pubsubTopicField is KMutableProperty<*>) {
      ...
      context.getStore(STORE).put(PUBSUB_TOPIC, pubsubTopic)
      pubsubTopicField.setter.call(testInstance, pubsubTopic)

      // Setup a Pub/Sub subscription
      val pubsubSubscriptionField =
          testInstance::class.memberProperties.find { it.name == "pubsubSubscription" }
      if (pubsubSubscriptionField != null && pubsubSubscriptionField is KMutableProperty<*>) {
        ...
        context.getStore(STORE).put(PUBSUB_SUBSCRIPTION, pubsubSubscription)
