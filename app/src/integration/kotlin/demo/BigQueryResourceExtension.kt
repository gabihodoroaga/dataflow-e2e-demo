...
class BigQueryResourceExtension : BeforeAllCallback, AfterAllCallback {
    ...
    companion object {
        ...
        val STORE = Namespace.create("com.nodle.bq")
        ...
        fun createBigQueryTable(
            ...
            try {
                ...
                val dataset = bigquery.getDataset(DatasetId.of(projectId, datasetName))
        ...
        fun deleteBigQueryTable(projectId: String, datasetName: String, tableName: String?) {
            ...
            try {
                ...
                val success = bigquery.delete(TableId.of(projectId, datasetName, tableName))
                if (!success) {
                    logger.info(
            ...
            return GsonBuilder()
                    .registerTypeAdapter(
                            ...
                            JsonDeserializer<TableFieldSchema>({
                                ...
                                val fieldSchema =
                                        Gson().fromJson(item, TableFieldSchema::class.java)
                                if (fieldSchema.fields != null) {
                                    fieldSchema.fields =
                                            context.deserialize<Array<TableFieldSchema>>(
        ...
        fun getTableSchema(jsonData: String): Schema {
            ...
            val fieldsList =
                    getGson().fromJson(jsonData, Array<TableFieldSchema>::class.java).toList()
    ...
    override fun beforeAll(context: ExtensionContext) {
        ...
        bigQueryTableFields.forEach {
            if (it is KMutableProperty<*>) {
                val bigQueryDataset =
                        Utils.readInstanceProperty<String>(testInstance, "bigQueryDataset")
                ...
                val bigQueryTable =
                        createBigQueryTable(projectId, bigQueryDataset, tableName, tableSchema)
                if (bigQueryTable == null) {
                    fail(
                            "Cannot create BigQuery table for project $projectId, tableName $tableName"
                    )
                }
